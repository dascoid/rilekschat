...
const onSubmit = async () => {
  ...
  if (validation.valid) {
    try {
      const res = await useApi('/send_broadcast_chat', {
        method: 'POST',
        body: JSON.stringify({
          templates_uuid: templates.value,
          bot_id: props.botId,
          whatsapp_users: props.waUsers,
...
const listTemplatesApi = await useApi(createUrl('/list_templates_chat'), {
  method: 'POST',
  body: JSON.stringify({
    bot_id: props.botId
  }),
            ...
            <VRow>
              ...
              <VCol cols="12" v-if="selectedTemplate">
                ...
                <!-- <VCard variant="tonal" class="pa-4">
                  ...
                  <div class="mb-2">
                    <strong>Body:</strong>
                    <p class="mb-0">{{ selectedTemplate.whatsapp_templates_body }}</p>
                  </div>
